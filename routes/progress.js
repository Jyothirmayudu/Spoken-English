...
function parseProgressRow(row){
  if(!row) return { completed: [], levelScores: {}, quizStats: {}, subjectStats: {}, unlocked: [1] };
  return {
    ...
    subjectStats: JSON.parse(row.subject_stats_json || '{}'),
...
router.put('/progress', requireAuth, (req, res) => {
  const { completed, levelScores, quizStats, subjectStats, unlocked } = req.body || {};
  ...
  const subjectStatsJson = JSON.stringify(subjectStats && typeof subjectStats === 'object' ? subjectStats : {});
  ...
  if(existing){
    db.prepare(`
      UPDATE progress
      SET completed_json = ?, level_scores_json = ?, quiz_stats_json = ?, subject_stats_json = ?, unlocked_json = ?, updated_at = datetime('now')
      WHERE user_id = ?
    `).run(completedJson, levelScoresJson, quizStatsJson, subjectStatsJson, unlockedJson, req.user.id);
  }else{
    db.prepare(`
      INSERT INTO progress (user_id, completed_json, level_scores_json, quiz_stats_json, subject_stats_json, unlocked_json)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(req.user.id, completedJson, levelScoresJson, quizStatsJson, subjectStatsJson, unlockedJson);
  }
